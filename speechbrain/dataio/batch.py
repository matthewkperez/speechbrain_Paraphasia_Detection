        ...
        for i in range(len(self)):
            ...
            for key in self.__keys:
                ...
                if isinstance(value, PaddedData):
                    data = value.data[i]
                    lengths = value.lengths[i]
                    
                    pad_length_compute = lengths * data.size(0)
                    # print(f"pad_length_compute: {type(pad_length_compute)}")
                    # print(f"round: {round(pad_length_compute.item())}")
                    # exit()
                    unpadded_data = data[:round(pad_length_compute.item())]
                    value = torch.tensor(unpadded_data)
                else:
                    value = value[i]
