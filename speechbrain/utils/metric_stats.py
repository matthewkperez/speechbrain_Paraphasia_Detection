...
class ErrorRateStats(MetricStats):
    ...
    def summarize_dist(self, str_suffix, field=None):
        """Summarize the error_rate and return relevant statistics.

        * See MetricStats.summarize()
        """
        # print(f"self.scores wer: {self.scores}")
        tmp_dir = "ddp_json_collection_files"
        os.makedirs(tmp_dir, exist_ok=True)
        with open(f'{tmp_dir}/scores_rank{dist.get_rank()}_{str_suffix}.json', 'w') as f:
        ...
        if dist.get_rank() == 0:
            ...
            for rank in range(dist.get_world_size()):
                ...
                with open(f'{tmp_dir}/scores_rank{rank}_{str_suffix}.json', 'r') as f:
                    json_file = json.load(f)
                    all_scores.extend(json_file)
                    # print(f"WER json: {json_file}")
                    # print(f"all_scores: {all_scores}")
                    # print(f"all_scores1: {len(all_scores)}")
    ...
    def filter_dups(self,all_scores):
        ...
        Remove these
        '''
        # print(f"all_scores WER: {all_scores}")
        tracker = {} # key: count
        result = []
        for d in all_scores:
...
class BinaryMetricStatsF1(MetricStats):
    ...
    def append(self, ids, scores, labels):
        ...
        info = {'ids': ids[0], 'pred': scores, 'labels': labels}

        self.scores.extend([info])

    def summarize_dist_f1(
        self, device, field=None, threshold=None, max_samples=None, beta=1, eps=1e-8
    ):
        """
        ...
        with open(f'{tmp_dir}/f1_scores_rank{dist.get_rank()}.json', 'w') as f:
            json.dump([score for score in self.scores], f)
        ...
        if dist.get_rank() == 0:
            ...
            for rank in range(dist.get_world_size()):
                # Read scores from file
                with open(f'{tmp_dir}/f1_scores_rank{rank}.json', 'r') as f:
                    json_file = json.load(f)
                    all_scores.extend(json_file)
                # all_scores.extend(json_file)
            all_scores = self.filter_dups(all_scores)
            ...
            self.true = [lbl for d in all_scores for lbl in d['labels']]
            self.pred = [pred for d in all_scores for pred in d['pred']]

            pred = torch.tensor(self.pred).int()
            true = torch.tensor(self.true).int()
    ...
    def write_stats(self, filestream):
        ...
        * See MetricStats.write_stats()
        ...
        print(f"F1: {self.summary['F-score']}\n", file=filestream)
        print(f"True: {Counter(self.true)}\n", file=filestream)
        print(f"Pred: {Counter(self.pred)}\n", file=filestream)
