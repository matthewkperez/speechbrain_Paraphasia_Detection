#!/usr/bin/env/python3
"""
Paraphasia prediction is done by mean pooling token representations over the word frames
...
class ASR(sb.Brain):
    def tok2word(self,p_seq, out_ASR):
        # return word mask
        predicted_token_ids = p_seq.argmax(dim=-1)
        predicted_tokens_batch = [[self.tokenizer.id_to_piece(int(id)) for id in seq] for seq in predicted_token_ids]

        # Initialize a list to store mean-pooled features
        mean_pooled_features_batch = []
        for b, predicted_tokens in enumerate(predicted_tokens_batch):
            # Initialize variables to keep track of word boundaries and mean-pooled features for this sequence
            current_word_tokens = []
            ...
            for i, token in enumerate(predicted_tokens):
                ...
                if token.startswith("▁"):
                    if len(current_word_tokens) > 0:
                        # new word and write old word
                        mean_pooled_features = torch.mean(torch.stack(current_word_features), dim=0)
                        print(f"mean_pooled_features: {mean_pooled_features.shape}")
                        utt_pooled_features.append(mean_pooled_features)

                        current_word_tokens = [token]
                        current_word_features = [out_ASR[b, i, :]]
                    else:
                        # add word
                        current_word_tokens.append(token)
                        current_word_features.append(out_ASR[b, i, :])
                    ...
                    # existing word
                    # Add the current token and its corresponding features to the lists
                    current_word_tokens.append(token)
                    current_word_features.append(out_ASR[b, i, :])

                # if token == '▁' or i == len(predicted_tokens) - 1:
                #     print(f"{token} | {i} = {len(predicted_tokens) - 1}")
                #     # Perform mean pooling on the features of the tokens that make up the current word
                #     mean_pooled_features = torch.mean(torch.stack(current_word_features), dim=0)
                #     print(f"mean_pooled_features: {mean_pooled_features.shape}")
                #     utt_pooled_features.append(mean_pooled_features)

                #     # Reset the lists for the next word
                #     current_word_tokens = []
                #     current_word_features = []
            ...
            if len(current_word_tokens) > 0:
                ...
                print(f"mean_pooled_features: {mean_pooled_features.shape}")
                utt_pooled_features.append(mean_pooled_features)
            print(f"utt_pooled_features: {len(utt_pooled_features)}")
            mean_pooled_features_batch.append(utt_pooled_features)
        
        # pad
        max_len = max([len(i) for i in mean_pooled_features_batch])
        pad_tensor = torch.zeros(out_ASR[b, i, :].shape)
        print(f"max_len: {max_len}")

        # for seq in mean_pooled_features_batch:
        #     if len(seq) < max_len:
        #         print(f"seq: {len(seq)}")
        #         seq = seq+[pad_tensor]*(max_len-len(seq))
        #         print(f"seq: {len(seq)}")
        #         exit()

        # exit()
        mean_pooled_features_batch = [torch.FloatTensor(seq+[pad_tensor]*(len(seq)-max_len)) if len(seq) < max_len else seq for seq in mean_pooled_features_batch]
        mean_pooled_features_batch = torch.stack(mean_pooled_features_batch)
        print(mean_pooled_features_batch.shape)
        exit()


        return mean_pooled_features_batch
    ...
    def compute_forward(self, batch, stage):
        ...
        # Word boundary pooling
        self.tok2word(p_seq,out_ASR)
