...
def compute_f1_score_recall_n(true_labels, predicted_labels, n=0):
    ...
    for i, (true_label, predicted_label) in enumerate(zip(true_labels, predicted_labels)):
        ...
        # GT=0 and all preds 1's
        # elif all(label == 1 for label in neighborhood):
        #     FP += 1
...
def change_yaml(yaml_src,yaml_target,data_fold_dir,frid_fold,para_type,output_neurons,output_dir,w2v_model):
    ...
    speaker_emb = False
    # frid_fold = data_fold_dir.split("_")[-1]
    # para_type=['pn','p','n'][1]
    # output_neurons=500
    ...
    # base_model = f"results/Table2/S2S-PT/{w2v_model}/pn-{output_neurons}-unigram"
    base_model = f"results/Table3-token_sweep/S2S-PT/{w2v_model}/pn-{output_neurons}-unigram"
    ...
    with open(yaml_target) as fin:
        ...
        filedata = filedata.replace('freeze_ASR_PLACEHOLDER', "True")
...
if __name__ == "__main__":
    ...
    EXP_DIR = f"results/Table2/S2S-FT/{W2V_MODEL.split('-')[0]}/{PARA_TYPE}-{OUTPUT_NEURONS}-unigram"
    # EXP_DIR = f"results/Table3-token_sweep/S2S-FT/{W2V_MODEL.split('-')[0]}/{PARA_TYPE}-{OUTPUT_NEURONS}-unigram"
    ...
    if EVAL_FLAG:
        ...
        base_utt_recall = recall_score(y_true, y_maj_class, average='binary')
